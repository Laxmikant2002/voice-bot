...
export class KmsUtil {
  ...
  async decryptApiKey(encryptedKey: string): Promise<string> {
    try {
      ...
      return Buffer.from(decrypted.Plaintext as Buffer).toString('utf-8');
    } catch (error) {
   ...
   */
  async encryptApiKey(plaintextKey: string, keyId: string): Promise<string> {
    try {
      if (!plaintextKey) {
      ...
      return Buffer.from(encrypted.CiphertextBlob as Buffer).toString('base64');
    } catch (error) {
